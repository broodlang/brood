...
//! fixed; this is what keeps them fixed. Runs the checker four times (~40 s debug) and is
//! worth it: a cache that changes a verdict is a checker bug nothing else would catch.
...
#[test]
fn the_site_walk_cache_changes_no_verdict_and_no_inferred_signature() {
    let root = workspace_root();
    for tree in ["std", "tests"] {
