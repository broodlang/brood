...
//! expect: `fso` matches `format/source` via f…s…o). The client typically filters
...
mod tests {
    ...
    fn subsequence_matching() {
        // Fixture data for a generic string algorithm — `matches` knows nothing
        // about Brood names, so these literals are NOT references to anything and a
        // rename sweep must leave them alone (ADR-236's sweep rewrote them to
        // "source", which silently made every positive case unsatisfiable).
