...
fn write_sandbox_rejects_a_symlink_escape() {
    ...
    let ok = interp
        .eval_str("(try (do (mcp/project-path \"src/real.txt\") :ok) (catch e :blocked))")
        .unwrap();
    assert_eq!(
        interp.print(ok),
        ":ok",
    ...
    let ok2 = interp
        .eval_str("(try (do (mcp/project-path \"src/new.blsp\") :ok) (catch e :blocked))")
        .unwrap();
    assert_eq!(
        interp.print(ok2),
        ":ok",
    ...
    let blocked = interp
        .eval_str("(try (do (mcp/project-path \"evil/passwd\") :ok) (catch e :blocked))")
        .unwrap();
    assert_eq!(
