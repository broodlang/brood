...
fn reactor_death_errors_instead_of_hanging() {
    ...
    let lid = brood::host::net::listen("127.0.0.1", 0, 999_999).expect("listen before death");
    assert!(
        brood::host::net::local_port(lid).is_some(),
        "listener registered"
    );

    brood::host::net::die_for_test();
    ...
    loop {
        if brood::host::net::listen("127.0.0.1", 0, 999_999).is_err() {
    ...
    assert!(
        brood::host::net::connect("127.0.0.1", 1, 999_999).is_err(),
        "connect must error once the reactor is dead"
    );
    assert!(
        brood::host::net::send(lid, b"x").is_err(),
