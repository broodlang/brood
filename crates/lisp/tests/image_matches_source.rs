...
//! value — it has been a missing name, a lost attribute, or a wrong kind. The per-module
//! REGISTRATION half (KI-136) is `image_registrations_match_source.rs`, a binary of its own.
...
fn own_the_cache() -> std::path::PathBuf {
    ...
    // Safe here: this binary holds one test (`env_isolation` enforces it), so nothing else
