...
/// Does this `reduce`/`fold` call carry a **failure into its own accumulator** that the
/// callback does not absorb? The fold's step function is written for the accumulator it
/// builds; a step that can return a `failure` hands one back as the NEXT step's
/// accumulator, and unless the callback answers for that case the fold does something
/// its author did not write — in the motivating bug, `(conj <failure> x)`, which raises.
///
/// The test is the fixpoint the result inference above already computes, plus one
/// condition. Instability ALONE is not a bug — it is routinely just the limit of the
/// analysis, which is why that path silently widens to the `any`-seeded reading. But an
/// accumulator that gained a `failure` AND does not stabilise is different:
///
/// - guarded (`((acc _) :when (failure? acc) acc)`) → the step returns the accumulator
///   unchanged for the failure member, so `f(acc₁,e) ⊆ acc₁` holds, it stabilises, silent.
/// - unguarded → the failure is fed back into a step with no case for it, the result
///   escapes `acc₁`, and that is exactly the program that breaks on the next element.
///
/// So the pair (gained a failure, did not stabilise) is the signal, and neither half
/// alone would be.
pub(super) fn fold_leaks_failure(heap: &Heap, items: &[Value], ctx: &Ctx) -> bool {
    let Some((coll_arg, f)) = super::sigs::combinator_args(items) else {
        return false;
    };
    // Only the explicit-init forms: `(reduce coll init f)` / `(fold coll init f)`. The
    // 2-arg `(reduce coll f)` seeds from the first element, where there is no separate
    // accumulator type to reason about.
    if items.len() != 4 {
        return false;
    }
    let Some(init_ty) = expr_ty(heap, items[2], ctx) else {
        return false;
    };
    let elem = expr_ty(heap, coll_arg, ctx)
        .and_then(|t| t.elem_ty())
        .unwrap_or(Ty::ANY);
    // Only an INLINE `fn` literal, and only one that never mentions `failure?`.
    //
    // Both halves were measured, not assumed. The tempting rule — "unstable fixpoint plus
    // a failure in the accumulator" — reads as if a guarded callback would stabilise and
    // stay silent. It does not: `callback_ret` types EVERY clause against the widened
    // accumulator, including the ones a `:when (failure? acc)` guard excludes, so the
    // guarded arm's siblings still see a failure, still go wide, and the guarded program
    // warns exactly like the unguarded one. That is a false positive on the fix the
    // warning itself recommends, which is the worst kind.
    //
    // So the suppressor is syntactic and deliberately blunt: an author who has written
    // `failure?` anywhere in the callback has considered the case. And a callback passed
    // by NAME is out of scope entirely — its body is not in hand here, and guessing from
    // an inferred arrow reproduced the same false positive.
    let failure_pred = value::intern("failure?");
    let inline_fn = super::walk::fn_form_items(heap, f).is_some();
    if !inline_fn || super::walk::sym_appears_in(heap, f, failure_pred) {
        return false;
    }
    let Some(step) = callback_ret(heap, f, &[Some(init_ty.clone()), Some(elem.clone())], ctx)
    else {
        return false;
    };
    let acc = init_ty.union(step);
    // The accumulator has to actually contain a failure for this to be that bug.
    if !Ty::of(Tag::Failure).is_subtype(&acc) {
        return false;
    }
    match callback_ret(heap, f, &[Some(acc.clone()), Some(elem)], ctx) {
        // Stable — the callback absorbs the failure (a guard clause, or it returns it
        // unchanged). Nothing to say.
        Some(again) => !again.is_subtype(&acc),
        // The callback could not be typed against the widened accumulator at all, which
        // is an analysis limit rather than evidence of the bug.
        None => false,
    }
}
