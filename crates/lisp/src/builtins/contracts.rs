...
//!    below its definition), and `sig!`'s explicit `%contract-install!`;
...
pub(crate) enum OnShim {
    ...
    Rewrap,
    /// Leave it — the sweep and `%contract-install!`, whose declarations did not change.
    Keep,
}
...
/// `(%contract-install! 'name)` — offer `name`'s binding to the policy now (`sig!`'s
/// explicit step, after it has forced the name); true when a shim was installed.
fn contract_install_prim(args: &[Value], _: EnvId, heap: &mut Heap) -> LispResult {
    let name = expect_symbol(heap, "%contract-install!", arg(args, 0))?;
    let name = crate::eval::macros::resolve_reference(heap, name);
    Ok(Value::boolean(contract_apply(heap, name, OnShim::Keep)?))
}
