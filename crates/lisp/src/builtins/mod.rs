...
static PRIMITIVE_DOCS: &[(&str, &[&str], &str)] = &[
    ...
    ("%module-direct-requires", &["path"], "Parse the file at path (no eval) and return `{:module <name-or-nil> :requires [<module-name> …]}` — its own module name and the modules it directly `:use`s / `:use-internals`. The edge list `project.blsp` closes transitively into each file's check-file reachability set (KI-17)."),
    ...
    ("scan-source-extract", &["src"], "Native per-file scan for the whole-project check (ADR-119): parse src and return [counts privs def-names] — a map of every symbol's occurrence count, this file's `defn-`/`def-` privates as [bare qual], and every top-level def's qualified name. The fast path replacing the interpreted CST walk."),
    ...
    ("private?", &["name"], "Whether the global `name` is module-private (ADR-146). Quote the qualified symbol: (private? 'mod/helper)."),
    ...
    ("builtin-modules", &[], "The names of every module baked into this binary, as a sorted list of strings — what a `name/…` reference or `(:use name)` resolves without a load-path. Backs `nest` shell completion and lets a name be validated before referencing it."),
