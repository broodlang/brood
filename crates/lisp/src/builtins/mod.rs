...
static PRIMITIVE_DOCS: &[(&str, &[&str], &str)] = &[
    ...
    ("scan-source-extract", &["src"], "Native per-file scan for the whole-project check (ADR-119): parse src and return [counts privs def-names] — a map of symbol counts, this file's `defn-`/`def-` private defs as [bare qual], and every top-level def's qualified name. The fast path replacing the interpreted CST walk."),
