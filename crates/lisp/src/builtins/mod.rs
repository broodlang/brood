...
pub fn register(heap: &mut Heap, root: EnvId) {
    ...
    // encode/decode pair per container format. The Erlang `:zlib`-shaped public names
...
static PRIMITIVE_DOCS: &[(&str, &[&str], &str)] = &[
    ...
    ("%gzip", &["bytes"], "gzip-compress a byte sequence (RFC 1952, the `Content-Encoding: gzip` wire format), returned as a bytes value. The one gzip primitive; the Erlang-shaped `zlib/gzip` is Brood over it in std/zlib.blsp."),
