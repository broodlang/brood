...
| `BROOD_SPAWN_SPILL=<n>` | Backlog at which a spawn stops going to the **spawner's own worker** and spills round-robin (`process/scheduler.rs`) — default **1**, so a child stays local only while our queue is empty. Placement used to be always-local, which meant a dispatcher spawning a handler per request piled every one onto its own queue where a slow handler blocked the rest; stealing rebalanced only 12%. Measured on the `latency` row (median of 5, p50/p99/p99.9 µs): always-local 141/674/2902 · spill 8 78/457/3354 · spill 4 62/397/3852 · spill 2 48/289/– · **spill 1 27/232/562** · always-RR 12/168/3864. A huge value restores the old always-local behaviour for an A/B. |
