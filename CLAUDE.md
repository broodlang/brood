...
| `BROOD_NO_TW_REENTRY=1` | **Opt-OUT** of the tree-walker→VM router (default ON since 2026-08-30): a VM-eligible closure invoked from TREE-WALKED code runs through `vm_apply` instead of inheriting its caller's ~10× engine. Before this, `apply_closure` never re-entered the VM, so ONE deferred call tree-walked everything beneath it transitively — the mechanism behind §7.3's expansion constant. Bounded by `TW_REENTRY_BUDGET` (32): past it the call tree-walks as before, so mixed-eligibility mutual TAIL recursion stays flat (`tests/tw_reentry_test.blsp` pins 300k alternations). Measured: 60× on an eligible helper hammered from an autogensym-deferred driver; `startup` −6.9%; standard rows neutral (pingpong solo +1.0%, instructions −9M). The A/B / bisect lever. |
