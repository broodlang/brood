...
| `MIMALLOC_PURGE_DELAY=0` | Not ours — mimalloc's own env option, worth knowing. The allocator holds freed pages by default, so RSS on a churny workload sits ~2.5× above the live working set (measured 2026-07-30: 208 MB vs **90 MB** on the same soak, 83 MB also setting `MIMALLOC_PURGE_DECOMMITS=1`), costing ~4% throughput. Live data in that run was 59 KB — **RSS is not a proxy for live bytes on this runtime.** The default is the deliberate "spend memory for speed" choice (devlog 2026-06-15); this is the lever if a deployment wants the memory back. See `docs/runtime-frontier.md` A8. |
