...
| `BROOD_JIT_INLINE=1` | **Opt-in** (default OFF) for the JIT recursive self-inliner (Phase B, `docs/jit-optimizing-tier.md` §6b). Shelved default-off because it inflates the shared VM body and regresses non-tiering workloads (spawn ~6.5×, bintree ~4.8×) — re-enable only for A/B until the JIT-only-body fix lands. **Needs `--features jit`**; `BROOD_INLINE_DBG=1` traces which arms inline. |
