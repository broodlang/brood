...
| `BROOD_STDIMAGE=1` | **Opt-IN** to the stdlib **startup image** (ADR-256). A `require`d std module materialises its bindings from `~/.cache/brood/std-image-<stdlib-id>.bin` instead of re-reading and re-evaluating its source: `json` 6.5 → 1.7 ms, `http` 12.0 → 3.6 ms, the `json` benchmark row −5.6%. Safe by construction — the key is a **content hash** of every baked-in `.blsp`, so a stale image cannot be read, and with none present `install` returns nil in ~30 µs. The runtime never BUILDS one (~1 s, which would land on the short-lived runs it helps); **`nest` writes it** when this flag is set. **Not the default, and the reason is KI-72:** under parallel load it turns a latent require-stall into a reliable one (12/12 parallel `autoload_race` copies over a 90 s cap with it, 0/12 without). Also stands aside under `BROOD_COVERAGE` — coverage instruments the compiler, and a materialised module is never compiled. |
