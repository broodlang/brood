...
| `BROOD_NO_CHECK_CACHE=1` | Bypass the incremental `nest check` result cache (ADR-129) **and `brood file`'s run pre-flight verdict cache** (ADR-371, `~/.cache/brood/run-check/`) — recheck everything from scratch. Reach for it when a checker change is in flight and cached results would mask it. The `nest check` half is `std/tool/project-check.blsp`; the run half is `cli_support::run_check_cache_*`, keyed on the source text + `system/build-id` + `system/stdlib-id` + the checking mode, and it declines to record a verdict that loaded a load-path module. `brood --check` never reads it. |
