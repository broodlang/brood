...
| `BROOD_STDIMAGE=1` | **Opt-IN** to the stdlib **startup image** at boot (ADR-256): `require` materialises a module's bindings from `~/.cache/brood/std-image-<stdlib-id>.bin` instead of reading and evaluating its source — `json` 6.5 → 1.7 ms, `http` 12.0 → 3.6 ms, the `json` benchmark row −5.6%. Needs the image to have been BUILT (`nest stdimage`); absent or stale it silently falls back to source, since the key is a content hash of what is baked in and cannot outlive it. Installed from the PRELUDE, because that is the only place earlier than the first `require` — installing from a program materialises **nothing** (the framework's own qualified names have already pulled the library in from source at compile time), which is how three separate published failure counts came to be measured against the source path. Suite is 4917/4917 either way. Off by default: it changes every program's load path, and flipping it is its own decision. |
