...
crates/lisp/src/   (the directory tree mirrors the layers — see lib.rs)
  ...
  boot.rs + boot/   how a runtime comes to hold the prelude: the shared `SHARED` bundle,
               the two boot paths (prelude image ADR-314 / source),
...
std/                     standard library written in Brood, grouped (ADR-085):
                         ...
                         prelude/ (split across ten bare-root files (core, predicates, map, …, contracts last), concatenated in the order boot.rs lists) + ~50 bare-core modules (io, file, set, regex,
...
| `BROOD_TRACE_GLOBAL=<name>` | Narrate every root write to that ONE global — `define`, `swap->staged`, `swap->LIVE` — with the writer's pid and isolate scope, plus its value **before and after** every globals restore and the load journal's mark / outstanding count / entries for it. The KI-141 tool: a registry that is full in one test file and empty in the next has a timeline nothing else prints (`BROOD_REG_TRACE` watches `*record-ids*` alone). Read once; free when unset. `BROOD_TRACE_GLOBAL='*faces*' BROOD_NO_STDIMAGE=1 cargo nextest run -E 'binary(suite)' --no-capture` is the invocation that found it. |
...
| `BROOD_TRACE_COMPILE=1` | Name every closure the VM **bytecode-compiles**, with its cache key and body region (`[compile] id_region=… key=Body(…) body=…`). The question it answers: is a compiled body being *reused*? A key repeated once per process means the shared cache is missing — which is exactly how ADR-215 was found (100 154 compiles for 100 000 spawned processes, 8.1 µs each). Works in release; compiles are rare once sharing works, so the check is free. Since 2026-09-17 it also prints `[compile] stale-bindings arm=…` when a miss inside a compiled arm (or the tier-up pre-load) LOADS a module — that arm was compiled without the module's bindings and recompiles at its next activation (ADR-366, KI-163). |
...
| `BROOD_JIT_BAIL_TRACE=1` | Name each arm a lowering **refuses or demotes**, with the reason (`[jit-bail] arm=<name> reason=…` — the profitability gate's `call-mediated-boxed` (`jit_plan::plan_general_lowering`), pre-checks like `chunk-outside-jit-subset` and `hosts-receive` (KI-168: a NON-tail call site reaches a `receive` through named callees, so the arm would park its worker dirty under a native gateway — refused before compiling instead of latched after), mid-emit refusals like `call-spill-exhausted`, and runtime demotions like `deopt-thrash-latched` (with its `last-deopt` reason) and `suspend-latched`), and each re-tier deopt feedback asks for (`[jit-relower] … reason=polymorphic-param \| float-through-erased-reads`, ADR-378). The complement of `BROOD_JIT_DUMP_IR`: that tool shows what lowered, so a refusal was only ever visible as *absence* — indistinguishable from an arm that was never hot or never tried. Reach for it when a row is slower than expected and you want to know whether the hot arm was rejected on purpose. **Needs `--features jit`**; one cached `var_os` when off. |
...
| `BROOD_DEOPT_TRACE=1` | Print each JIT type-deopt's arm name + `deopt_watch` flag + **deopt-site id** to stderr (`[deopt] arm=advance-body watch=true resume_ip=… reason#106`) — for finding which arm keeps falling off the native path (the deopt-feedback / matmul-class signal). `reason#N` is the `__dr` constant the failing guard passes to the deopt block: grep `iconst(types::I32, N)` under `jit_lower/` for the guard (106 = a register-carried param's entry tag check — a polymorphic param, which `jit_any_deopt_feedback` re-lowers boxed after sixteen). A persistent deopt used to name its arm and nothing else; `json/emit`'s 4 819 per run were found by the id. **Needs `--features perf-stats`.** The `:deopts-total` of `(%jit-arm-state f argc)` counts the same events in any dev-tools build — read THAT for a `SelfCall` loop, whose `:deopts` (the feedback's consecutive count) never moves. |
...
| `BROOD_NO_MAPGET=1` | **Opt-OUT** of the `(get m k)` → `PrimOp::MapGet` lowering (ADR-296) — **default ON since 2026-09-17**. A 2-arity map read is one CHAMP probe in the VM and one callback in native code; a plain map's miss is answered inline too (`Heap::map_get_inline`) and only a **record's** miss declines to the prelude `get` (its `%lookup-miss` → `Lookup` ability). Measured: a hit 322 → 66 ns, a 100% miss 519 → 123 ns (the opt-in version made a miss +200 ns, which is why it shipped opt-in), `supervisor` −8%. The A/B and bisect lever; `crates/cli/tests/mapget_differential.rs` compares every branch of `get` against this flag at every tier. Since ADR-368 the same lever also covers the 3-arity `(get m k default)` (`PrimOp3::MapGet3`). |
| `BROOD_NO_MAPASSOC=1` | **Opt-OUT** of the `(assoc m k v)` → `PrimOp3::MapAssoc` lowering (ADR-368) — **default ON since 2026-09-17**. A single-pair `assoc` on a map is the kernel path-copy directly (the VM) or one `brood_rt_map_assoc` callback (native); a vector receiver declines to the wrapper. Its own lever, apart from `BROOD_NO_MAPGET`, because this one ALLOCATES from native code where the reads do not — a suspected fault in either bisects without losing the other. Measured: a 2M-iteration `assoc` loop 921 → 590 ms (460 → 295 ns), `(get m k d)` 281 → 43 ns per read. `crates/cli/tests/mapprim3_differential.rs` compares every branch of both wrappers against the flags at every tier. |
...
| `BROOD_NO_INT_SLOT_CACHE=1` | **Opt-OUT** of the JIT's per-slot **i64 cache** (`Frame::slot_i64_cache`, 2026-09-25) — default ON. A read of a slot the lowering just stored an unboxed int to returns that SSA value instead of reloading the tag, comparing it to `Int`, branching to `deopt` and reloading the payload, which every read of a `let`-bound int paid (Cranelift does not forward the store): a three-binding int loop 314 → 157 ms, the benchmark rows flat. The twin of the f64 cache and maintained in the same two places, so it has the same soundness argument. The A/B and bisect lever; `tests/jit_int_slot_cache_test.blsp` guards it. **Needs `--features jit`**. |
| `BROOD_FLOAT_VETO=1` | Restore the pre-2026-09-20 **float-slot veto** in the profitability gate (`jit_plan::plan_general_lowering`): a named self-tail arm with non-tail calls was refused the general lowering when its profile showed a `Float` param — priced under a runtime where the floats arrived boxed from a thrash-latched `->float` (KI-109). Re-measured (§7.9), the clause vetoed one arm in the corpus (`mandelbrot`'s `row-sum`) and protected none (`nbody`'s refusals are the self-loop test's); admitting it is `mandelbrot` −4.7% cycles at `BENCH_N=1400`, every other row flat. The bisect lever. |
...
| `BROOD_NO_IDLE_BACKOFF=1` | **Opt-OUT** of the **empty-pool park backoff** (`process/scheduler/pool.rs`) — default ON since 2026-09-09. A parked worker whose pool is provably empty (`STEALABLE == 0`) doubles its `wait_timeout` backstop from 10 ms up to 500 ms, resetting to 10 ms the moment it runs anything. **With work queued anywhere the cadence is unchanged**, which is the regime stealing and the stranded-work watchdog live in — set this flag to pin the old 10 ms-forever behaviour for an A/B or a bisect. The bug it fixes: an idle runtime woke every worker 100x/s to load one atomic and park again, costing **0.25% of a core per worker** — 6-8% on a 28-core box, doing nothing. Found as a `nest mcp` server that had burned **10 hours of CPU** while idle for five days. Measured, 28 workers idle 20 s: `user=0.43s sys=1.01s` before, `user=0.01s sys=0.01s` after. Post-idle fan-out latency is unchanged (a spawn wakes an idle peer directly — the timeout was never the discovery path), including with `BROOD_SPAWN_SPILL` huge, where only stealing parallelises. |
...
| `BROOD_CONTRACTS=1` | Arm **runtime type contracts** from `sig` declarations — the runtime counterpart of `nest check`'s static advice, and **`nest run`/`nest test`'s DEFAULT** since 2026-09-21 (ADR-381; `BROOD_CONTRACTS=0` opts a run out, a released bundle never arms). Enforcement is a property of the BINDING, not of the declaring form: the kernel offers a binding to the Brood policy `%contract-wrap` (`std/prelude/contracts.blsp`) at a `def`, at a `%register-sig` on a bound name, and in the sweep at `provide` — from source OR from the stdlib image — and binds the checking shim it returns, keeping name/doc/privacy/meta. So a `sig` may stand above or below its definition (the old placement gate is gone), a materialised std module IS contracted (it used to evaluate no sigs, so the mode was barely exercised with an image present — KI-113), and a hot reload re-wraps. A mismatch is `{:kind :contract :blame :caller\|:callee :function :argument :expected :got :message}`. **`=all`** also enforces the prelude's own sixteen declarations, which plain `1` exempts: they are the hottest names in the language (`nth` 0.4 → 4.5 µs shimmed, `json/decode` ×16). Cost, release: ~3.5 µs per contracted call, charged at the MODULE BOUNDARY only (below). `stdimage/build` REFUSES to run armed (an image carries what loading bound, and shims in it would enforce contracts in every unarmed run); `nest` builds its image child with the variable removed. `contracts_mode.rs` walks every baked-in module from source under it; `crates/nest/tests/contracts_default.rs` proves the default through the image. |
| `BROOD_NO_CONTRACT_BOUNDARY=1` | **Opt-OUT** of the contract **module boundary** (ADR-383) — default ON. A module's calls to its OWN contracted functions are unchecked: beside the shim the kernel binds the original under a private alias (`string/%orig%char-at`), the compiler resolves a same-module reference to it once per site (from the closure's authoring module, `Closure::module`, inherited by every closure an arm builds), and the pass-through redirect and the tree-walker unwrap a same-module shim by value; `sig!` has no boundary and a root name (a script's, the prelude's) none either. It is what `format_test`'s ×35 was: `string/format` paying for `string/char-at` per character — `format` ×20 000 armed 1 312 → 338 ms (223 unarmed). Set this to charge every call, as ADR-381 shipped: the A/B and bisect lever, and the sabotage of `tests/contract_test.blsp`'s boundary cases (3 of 5 go red). A stale alias — the globals rolled back under a cached arm — resolves to the public name on the miss path, so the flag's OFF state can only ever check more, never less. |
| `BROOD_NO_LAZY_LOAD=1` | **Opt-OUT** of **lazy module loading** (ADR-335) — **default ON since 2026-09-12**. A qualified reference `mod/name` is a promise the name is bound *when evaluated*: a function reference loads its module at the first call that reaches it (the global-lookup miss path, `derive::global_miss`, so a hit pays nothing), where before it loaded when the *referencing* file loaded. `nest complete -- te` went **72 → 20 ms** and from 62 materialised modules to five; `(io/puts "hi")` materialises `io` alone instead of eight. A macro head still loads at expansion, and so does a head into a module the startup image does not describe (only the image's v6 kind index knows a name is not a macro without loading it). Set this to restore the compile-time loads — the A/B and bisect lever, and the stopgap if a first-use load is ever implicated. Two things it does NOT change: the miss path stays live (a module materialised from an image had no compile pass here, so an image built lazily needs it under either policy), and the checker + `nest run --check-boot` are eager regardless. `BROOD_IMAGE_TRACE=1` lists what actually materialised. |
| `BROOD_NO_IMAGE_SIGS=1` | **Opt-OUT** of reading function **signatures from the stdlib image** (ADR-370) — **default ON since 2026-09-18**. The **signature index** — for every function std's SOURCE defines, its existence and arity, and for a declaration the checker reads AS-IS (public, one arrow, a return that is neither `any` nor a bare collection, spelled from the type grammar alone; `std_index::image_carried_sig`) the declaration itself — is a fact about the embedded sources (`check::std_signature_index`): the image footer CACHES it, and a process booted without an image scans the one module a question is about on demand, so the checker knows the same either way (the artifact matrix requires it; `BROOD_NO_STDIMAGE=1` is not a different checker). The checker then answers a callee's type, arity and unbound verdict from the footer, and the TRANSITIVE body scan (`materialise_referenced_modules`, ADR-340 — what a loaded module's bodies name) loads nothing for such a name, nor for a name already BOUND (a native under a module prefix, `file/stat`). What remains loads as before: the FILE's own references (the program is about to call them — deferring that load to the first call is an ADR-366 recompile, `base64` +37M), and transitively an undeclared function, a declared `-> any`, an overload, a private name — the checker reads those from the body. Measured on `(io/puts (str (os/env "HOME")))`: the pre-flight check materialised 9 modules for a program that runs 2 and now 7 (`file` and `path`, through `os/which`, stay out) — and the instruction count did NOT move (105.5M either way, release, `perf stat`): materialising a module from the image is cheap on this box, so what this buys is the smaller heap and the removed dependence of a verdict on what else happened to load, not a faster check. The check's own cost sits elsewhere (`stdlib_tree_hash` is the top symbol of a debug `--check`, 14%). Set this to restore the whole-module scan and every load — the A/B, bisect and differential lever: `crates/nest/tests/image_sigs_differential.rs` requires `nest check --strict --suggest-sigs` over `std/` + `tests/` to agree byte for byte with it set and unset (two footer shapes failed that gate before the predicate was narrowed: an INFERRED signature re-rendered from source, and a declared `-> any`), and `check/tests/image_sigs.rs` requires every carried type to be the loaded module's declaration and every authoritative declaration to be carried. `BROOD_IMAGE_TRACE=1` prints `[image] check materialises <module> for <name>` — one line per reference that holds a load open (every name, since 2026-09-20 — the load stays until the LAST of them has a type), i.e. the names to declare signatures for. **The trace must never be on the path of what it traces (KI-171):** ADR-370's first shape put the scan's load-set insert behind the flag, so the transitive scan ran only when traced, for two days, and the one test of it ran traced. Three more skips ride the same lever since 2026-09-20: a **curated** name (`sigs::curated_sig` — `seq/filter`, `io/puts`, `math/nan?` — read ahead of the footer and of inference, so the load bought nothing), a **type-variable** declaration (`math/max`'s `(& ?A -> ?A)`, whose flat reading is `-> any`; the footer carries it and `image_heap_sig_with_vars` resolves it per call), and a `%defseq` definition (indexed at all — `seq/filter` had no entry, not even an arity). `sig_of` reads the footer AFTER the primitives (a native is never in it) and BEFORE the curated table (a declaration outranks a curated entry whether it comes from the loaded module or the footer — `math/even?` is curated `number` and declared `int`). |
...
| `BROOD_NO_PRELUDE_IMAGE=1` | **Opt-OUT** of the **prelude image** (ADR-314) — **default ON since 2026-09-04**. A warm boot materialises the prelude's ~808 bindings from `~/.cache/brood/prelude-expanded-<id>.img` instead of reading and evaluating the 544 expanded forms: release boot **21.6 → 13.5 ms**, `make ab --floor` reads **`startup` −11.1%** (0.0% floor) with `pipeline` −13.5%, `sieve` −7.3%, `reduce` −7.1%, `strings` −6.8%, `errors-deep` −6.7% and **no regression on any of 30 rows**. Set it to fall back to the source boot (neither reading nor writing the image), for an A/B or to bisect a suspected bad artifact; CI's tree-walker job sets it so the source path keeps deliberate coverage. It is the ONLY boot-artifact knob: the ADR-138 expanded-prelude text cache that used to sit between image and source was deleted 2026-09-12, and the gensym floor it carried in its header now lives in the image's own header (`brood-prelude-image-v3`). **It shipped default-on twice before and was reverted the same day both times**, each on a fact the evaluation *records* rather than binds: KI-105 (a stale stdlib section directory restored from the image — `%std-image-reinstall!`) and KI-106 (the registry-name set was not carried, so a multi-file `nest check` lost every derived multimethod mirror). Both fixed with sabotage-verified guards; the boot differential compares the registry set; and **`make check-imaged` runs the project's own checker gate with the image on**, because every test written *for* the image passed while KI-106 was live. The cold boot is unaffected by design — it does the full source boot and writes both artifacts. Stands aside under `BROOD_COVERAGE` (a materialised binding is never compiled). The trap to know: this restores **bindings**, so anything the evaluation merely *recorded* — `defdyn` marks, def sites, `meta`, privacy, registry marks — has to be written explicitly; five of those were missed in a row while building it. |
| `BROOD_NO_CHECK_CACHE=1` | Bypass the incremental `nest check` result cache (ADR-129) **and `brood file`'s run pre-flight verdict cache** (ADR-371, `~/.cache/brood/run-check/`) — recheck everything from scratch. Reach for it when a checker change is in flight and cached results would mask it. The `nest check` half is `std/tool/project-check.blsp`; the run half is `cli_support::run_check_cache_*`, keyed on the source text + `system/build-id` + `system/stdlib-id` + the checking mode + the flags that change a walk (an explicit list), and it declines to record a verdict that loaded a load-path module. A hit replays the walk's LOADS as well as its warnings (the lazy alternative cost +6 MB peak RSS on `startup` — a stale-marked recompile, ADR-366). `brood --check` never reads it. |
| `BROOD_NO_DERIVE_CACHE=1` | **Opt-OUT** of memoising the checker's site walk per top-level form across Pass 2.9's rounds (`sigs::SiteCache`, 2026-09-17) — a form is re-walked only when its own derived parameters or a name in its reference closure moved; 73% of walks reused, `nest check` over `std/` 11.0 → 8.8 s. Verdicts are identical by construction and by the gate `nest::derivation_cache_differential` (both ways over `std/` + `tests/`, byte for byte — which is how KI-158, the fixpoint's own nondeterminism, was found). The A/B and bisect lever. `BROOD_DERIVE_DBG=1` prints per file where a check's time went (joint fixpoint, derivation, rounds, walks reused). |
...
1. Implement it (special form in `eval.rs`; a builtin in its `builtins/<domain>.rs` —
   ...
   `std/prelude/*.blsp` — ten bare-root files concatenated in the order `boot.rs`
   lists, NOT the single `std/prelude.blsp` that older docs name. **A prelude closure
   that calls a macro defined AFTER it is tree-walked for good** — the forward call
   stays unexpanded and the VM refuses the whole closure — so define below the macros
   you use; the contract machinery sat above `and`/`or`/`cond` for a year at 200× cost).
