...
def expr(rng,d):
    ...
    return f"(math/abs {expr(rng,d-1)})"
...
def program(seed):
    ...
    return (f"(defn f (x) {body})\n"
            f"(defn pick (i)\n"
            f"  (cond (< (math/rem i 4) 1) (- (math/rem i 200) 100)\n"          # int (neg+pos)
            f"        (< (math/rem i 4) 2) (* 1.0 (- (math/rem i 200) 100))\n"  # float
            f"        (< (math/rem i 4) 3) (* 4611686018427387904 (+ 1 (math/rem i 6)))\n"  # bignum (past i64)
            f"        else (math/rem i 13)))\n"                            # small int
            f"(defn lp (i acc)\n"
            f"  (if (= i 0) acc\n"
