...
**KI-173 is the one to know about.** A module the pre-flight check loaded ran WITHOUT the
...
later. (6) ✅ swarm's five asks landed (addendum 3): pixel text, sound on the seam, assets +
release features, camera fit, harness names. Next candidates: a second game to test the
engine as a dependency further; a binary asset section in the bundle if base64 modules
get heavy; moving `b2d-web` into std once bedit wants a tab client. (5) ✅ the browser frontend (2026-09-20, b2d `dc791ae`): `b2d-web`, an HTTP+SSE bridge in Brood over `editor/serve`'s attach protocol + a canvas painter — generic over any served app, so bedit could get a tab client by the same route. Running the RUNTIME in the tab (wasm) remains open and would reuse that painter.
...
**The two differentials were fixed twice, in parallel, and both halves are in.** `669c580c`
(the section below) pinned the tier and raised the budget; this side also **split each binary
along the claim it already made** — process vs order, `std/` vs `tests/` — because nextest's
cap is per case and each case was doing three or four whole-tree checks. 21–29 s per case
under the tree-walker job's own environment, from 42–51 s, so the 360 s budget now carries
about twice the headroom its comment was written for. Sabotage-verified (a per-call marker in
`check()`'s output reds all four cases). One correction worth keeping: the 1.6× tree-walker
ratio in those comments is the **`[profile.test]`** `nest`; the same check on the RELEASE
...
**KI-164 (a declared `(is T)` guard did not narrow) is FIXED, 2026-09-18** — one line in
`predicate_guard_ty` (`is_local` counted the file's own globals as shadowing locals), four
...
**map literal inside a hot arm bailed the arm** — `MakeMap` was not in the JIT subset — **DONE
2026-09-18**: lowered as the variadic `MakeVector` is (`brood_rt_make_map_n` over
`map_from_pairs`), 32-value cap, `tests/jit_makemap_test.blsp`; and `assoc`'s remaining
