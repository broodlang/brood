...
| | `process-info` | 1 | `(process-info pid)` → an Erlang-`process_info`-style snapshot map of a live local process — `{:id :node :name :status :mailbox :monitored-by}` (`:status` is `:running`/`:waiting`; `:name` nil if unregistered) — or `nil` for a remote/dead pid. Assembled from the registry/scheduler/monitor tables; `:parent`/`:memory` join once the kernel tracks them (ADR-051). The observer reads this |
