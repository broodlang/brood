...
| [testing.md](testing.md) | The **test framework** (`std/tool/test.blsp`, written in Brood): ExUnit / `mix test`-style `describe`/`test`, the assertions, the **parallel-by-default** execution model with `:serial`/`:isolated`, and how share-safe tallying works. |
...
| [supervision.md](supervision.md) | **Reverted.** A kernel-level supervisor with resume slots and mode-gated automatic retry shipped briefly (2026-05-28) and was stripped (2026-05-29, `e3d3a0d`) because it was the dominant contributor to the multi-thread scheduler race. Erlang-style let-it-crash is back; `spawn` + `monitor` are the userland substrate for supervisors written in Brood. The note covers what was tried, why it was reverted, and the ~10-line userland respawn pattern. ADR-039 (reverted). |
