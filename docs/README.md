...
| [testing.md](testing.md) | The **test framework** (`std/test.lisp`, written in Brood): ExUnit / `mix test`-style `describe`/`test`, the assertions, the **parallel-by-default** execution model with `:serial`/`:isolated`, and how share-safe tallying works. |
