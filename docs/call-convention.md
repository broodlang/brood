...
the caller. (Landed 2026-09-17 as rung A4's first half: the callee nils every local,
unrolled; the write-set bound is the second half.)
...
| A1 | the inline blob stops saving/restoring rows 2–6; the cold path restores `jit_force_vm`/env roots | the call loop's instruction count (target ≤ 120 from 218); `bintree`, `pfib`, `nqueens`, `pipeline` under `ab --floor` — **LANDED 2026-09-16**: callbacks assert activation state from the ctx (`JitCallCtx::assert_into`; `brood_rt_call_native_fl` asserts only depth + gateway and reads its env from the ctx; a global read asserts env + the global-IC cursor; `brood_rt_fastlink_base` reads its cursor from the ctx and writes nothing), the blob writes none of the six fields, and every Rust GATEWAY restores what a chain may have asserted (found the hard way: `jit_tier_in_frame` never raised the depth so never restored it, the depth leaked one per chain to 75, and every inline call fell to the callback path on `depth < 64` — `bintree` +30% until the gateway restored). The real-call loop: 237 → **213** instructions per call (call-free 56.5 subtracted); `make ab --floor --all`: `bintree` −7.4%, `nqueens` −3.5%, every other row inside its floor, `nbody` +2.3% (the per-builtin-call assert, consistent, under the veto). Target ≤120 not reached: the remaining ceremony is the guards (~30), the ctx build + latch (~20), the nil-fill loop (rung A4's item) and `read_out` (A3) |
...
| A4 | callee-prologue nil-fill by first-safepoint write set | call loop ≤ 50; GC stress + verify over every `jit_*` file, the KI-49 tuple matchers in particular — **FIRST HALF LANDED 2026-09-17**: the callee nils its own locals, unrolled (`jit_lower.rs`, three stores per slot from the first `let` binder to `nslots`, before the stack guard so a guard-trip deopt hands the VM a nil frame), and the inline blob's fill loop is gone (`call.rs` keeps the capacity guard and the `len = frame_end` store). A Rust gateway still `extend_roots_to_nil`s and the callee then stores nil over nil — one `memset` plus three stores per slot on a path that costs thousands. Counted: a 5M-call loop whose callee has six locals 380 → **321** instructions per call (~10 per slot, as the row predicted); a callee with no locals is unchanged (there was nothing to fill). Rows: `bintree` −4.5% (its `check-node` temporaries), every other row inside its floor. GC stress + verify over every `jit_*` file, the tier-transition, metamorphic and linmap fuzzers clean. **The second half — skipping a slot the body definitely writes before its first safepoint — is not done**: it needs the write-set analysis over the chunk, and it buys three stores per such slot |
