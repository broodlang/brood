...
| KI-107 | **`tests/eval_server_test.blsp` — "`:all` traces, and cannot exceed the spy cap" fails with an EMPTY `:spy`**, about 1 run in 17 standalone | 🔍 **WATCH — one mechanism found, fixed and measured 2026-09-06; rate 6.0% → 1.8%, NOT closed.** `eval-capturing`'s teardown called `debug/untrace-all`, which restores every entry in a registry that is shared globals — so one request's teardown stripped a wrapper a *concurrent* request had just installed, matching the recorded evidence exactly (wrapper installed then lost, no refusal for the failing test's own name). Teardown now restores only the names the request itself wrapped. Measured against a same-N baseline: **9/150 → 10/550**, χ² = 7.59, p ≈ 0.006, a ~3.3× reduction. Guarded DETERMINISTICALLY — a trace installed outside a request must survive that request's teardown; with `untrace-all` restored the guard fails on every run, not one in fifteen. The first reading (3/40 vs 1/100) looked like a clean kill and was not: a 40-run baseline cannot tell 7.5% from 2%. Still open at 1.8%; the per-test session baseline was re-tried and refuted a second time (3/150). Next: the parent-side `untrace-all`, and refcounting the registry so a shared wrapper is restored by its last user |
...
## KI-107 — `eval_server_test`'s `:all` cases are coupled through shared trace state 🔍 WATCH — mechanism found and fixed 2026-09-06, rate 6.0% → 1.8%, NOT closed
