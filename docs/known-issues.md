...
| KI-48 | JIT tail dispatch read past the roots stack — `root_at(9)` on a len-8 stack, twice on 2026-08-20, from `jit_dispatch_tail`. Not reproduced on demand | ⚠️ **OPEN / watching** — captured in `.brood_crash_dump`, no repro yet; `argc = n - top - 1` underflows on the same input, so a release build may take a huge `argc` instead of panicking |
...
**One OPEN item: KI-48** (JIT tail dispatch read past the roots stack, 2026-08-20, not yet reproduced — see its entry). Before that, no open items — KI-36 was reproduced and fixed 2026-08-19, KI-47 the same day. `main` is green on all five CI jobs at `c8dbf0ea` (run 32247618122) — the first fully green run since the ADR-230/231 namespacing merge. KI-44 (the `sqrt` call-site inline, worth ~1.8× on `nbody`) and KI-45 (the stale `examples/editor`) were both fixed 2026-08-17. KI-43 (a fixed-sleep race in the remote-attach test) was found and fixed 2026-08-14. KI-28 is **no longer a watch item — it recurred twice
...
**Status:** ⚠️ **open / watching (2026-08-20).** Two crashes, 4 s apart, captured in
`.brood_crash_dump`; not reproduced on demand.
