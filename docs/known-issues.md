...
**Options:** (a) the shim closures are generated code and should carry no positions — strip
form positions when `%contract-shim-*` builds them, or load that one module with positions
off; (b) `attach_vm_trace` skips the frame of a contract shim (`shim_original` already tells
one from a plain closure) and takes the next positioned site; (c) re-pin the test at 135 and
accept that a `sig!` mismatch's position names the shim. (a) keeps both KI-181's rule and the
answer a user wants; it is the one to take.
