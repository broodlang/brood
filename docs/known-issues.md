...
## KI-136 — the stdlib image replays registrations a module never made ⏳ WATCH 2026-09-13
...
```
(highlight-spans "(defn f () (str \"a\"))")
  got      ([16 19 nil])
  expected ([1 5 {:bold true, :fg :blue}] [12 15 {:fg :cyan}] [16 19 {:fg :green}])
```

`highlight_test` (18), `configs_test` (8), `observer_test` (3) and the executed doc
examples for `editor/configs` — every one an assertion about a face, every one answered
`nil`, which is what `(face :missing)` returns. So `*faces*` is EMPTY by the time they run.

**Not the type-system work.** The identical failure set is present at `5afd40bd`, before
ADR-347..351; the run before it (`ef19b9be`) has none of them. The commits in between
change nothing on the face path.

**What it takes to see it.** Both the WHOLE suite and `BROOD_NO_STDIMAGE=1` — the source
path this job deliberately carries (KI-78). No subset reproduces, under the job's exact
environment (`BROOD_VM=0 BROOD_NO_STDIMAGE=1 BROOD_NO_PRELUDE_IMAGE=1`):
...
an imaged run has them before anything else happens. Only the source path loads them when
the module actually loads — and only this job runs that way.

**The mechanism (KI-89 class).** `%isolate` restores the globals table to the snapshot it
took. A face registered AFTER that snapshot — because `editor/highlight` loaded lazily
(ADR-335), inside or after some other test's isolate — is rolled back with it, and every
later reader sees an empty registry. `BROOD_SCOPE_DBG=1` is set in the job's env and logs
**6003 restores**, each with one live process; nothing in that log names a mass rollback,
...
load the face-declaring modules eagerly, before any isolate can snapshot — the `:load`
clause (ADR-335) is the tool, as `editor/lexer` already uses it. Neither is attempted here
because the failure has not been reproduced outside CI; the diagnostic is armed so the
next red run carries the restore log with it.
