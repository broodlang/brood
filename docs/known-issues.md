...
(2) def sites decode on the first `def-site` question rather than at boot. Neither was needed: after the
...
78.4M instructions, and the 653d41d9 benchmark column has `startup` at 13.1 ms. The levers stay
written up for the day boot matters more; the KI is closed on the measurement.
