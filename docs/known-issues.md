...
| KI-137 | **a strict verdict can depend on which OTHER modules are loaded when the file is checked, so the strict gate over std/ flickers with check order** — `math/clamp` (declared `-> number`, body `(math/max lo (math/min hi x))`) is clean in a fresh process and warns "body yields ordered" once `datetime` is loaded: `math/max` had no declaration, so its return was the kernel `%max` contract (number or comparable record), and the record half is inhabited only after a module registers comparable records. `nest check` checks files in parallel over one heap, so whether `datetime` is loaded when `math.blsp` is checked is a race: red twice on 2026-09-13 (a merged tree, a clean origin build), green in seven runs after, red again from the pre-push hook | ✅ **FIXED 2026-09-14** — two halves. The gate: `nest check` now brings the heap to ONE state before it checks anything (`project-check/project-preload!` — the listed files' header closures, the modules their bodies name, then the ADR-340 materialisation to a fixpoint, via `%check-materialise-referenced!`), so a verdict is a function of the LIST, never of check order; verified with the `max`/`min` declarations removed: `[math datetime]`, `[datetime math]`, std sorted and std reversed all report the one warning where before they split 0/1. Guard `tests/check_preload_test.blsp` (two fixture modules record at load whether the other is provided; listed in reverse, the sorted preload order still shows; sabotage-verified). The symptom: `clamp` calls `%max`/`%min` directly (what `math/max`'s two-argument clause is), which the checker types as its operands' union; a declaration `(sig max (& ?A -> ?A))` was tried first and reverted — it widens the curated `ordered` signature (`declared_vs_curated` tests). Residual, in the checker: `math/max`'s INFERRED return is the registry-derived `ordered` cover, which grows with every module that registers `compare-to` records, so a body calling the closure with `number` operands reads `ordered` once `datetime` is loaded; the extremum rule already unions operands for the primitive — the closure path should reach it too |
...
module in the listed files' header closures, every module a listed file's body names with a
qualified reference, then the ADR-340 materialisation to a fixpoint — sorted, before the first
check, in the fresh, cached and structured paths alike. A file's own module is not loaded
unless its body names it (a test file's top level must not run under `nest check`). Verified
with the `max`/`min` declarations removed: `[math datetime]`, `[datetime math]`, std sorted and
std reversed each report exactly the one `clamp` warning, where before the same lists split
0/1. Cost: the strict gate over std/ 31 s, std+tests+examples 89 s (debug), the loads it does
being the ones the checks did anyway. Guard: `tests/check_preload_test.blsp`.

**What remains is a checker question, not a gate one.** With the declarations removed,
`clamp`'s verdict still differs by whether `datetime` is in the list — an unrelated module.
The original text below records the mechanism as far as it was traced.
...
**Done (symptom).** `clamp` calls `%max`/`%min` directly — exactly what `math/max`'s and
`math/min`'s two-argument clauses are — because the checker's extremum rule types the
PRIMITIVE as the union of its operands (`number` here), while the CLOSURES' inferred return
is the `ordered` cover. `(sig max (& ?A -> ?A))` was tried first and reverted: it widens the
curated `ordered` signature, which `declared_vs_curated::no_declared_std_sig_widens_its_curated_signature`
forbids for a reason (a string operand must still warn).

**Mechanism, settled.** `ordered` is a NAMED COVER (ADR-299): `number` plus every record
`compare-to` has a method for. `datetime` registers `compare-to` for its three records, so
loading it grows the cover; `math/max`'s inferred return is that cover, and `clamp`'s
declared `number` is then narrower than its body's `ordered` under strict. Not a race in the
checker's rules — a verdict that legitimately depends on the heap, read under two heaps.
