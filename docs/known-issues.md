...
and `name` is not in the forced table (`forced`: the `%*contract-forced*` global, a table, read
with `table::has`) — the hook's own first test, hoisted into the mechanism so the policy is
