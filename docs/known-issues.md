...
| KI-97 | **consolidated hardening gaps from the 2026-08-31 stability audit** — pre-auth handshake trickle DoS (per-read timeout, no total deadline: 128 slow sockets silently disable inbound dist), untimed blocking calls on scheduler workers (`proc-send`, `os/run-process` with inherited stdin, `read-line`, `%node-connect` DNS), thread-spawn panic classes (`Once` poisoning in the timer, `LIVE_EXECUTORS` stranding in `ensure_workers`, gossip thread-per-peer unwinding the dist acceptor), and smaller items | ⚠️ **OPEN 2026-08-31; item 1 FIXED, item 2 half-FIXED 2026-09-01** — the section carries the full list with file:line; none observed in the wild, all confirmed by reading. **Item 1 (the pre-auth handshake trickle DoS) is closed**: a whole-handshake `Deadline` shim on both sides + a rate-limited shed warning, sabotage-verified. **Item 2**: `run-process`'s inherited stdin (a `git` credential prompt pinned a worker uncatchably) and `%node-connect`'s unbounded DNS resolve are both closed, each sabotage-verified; `proc-send`'s `write_all` and `read-line`'s stdin lock remain. Items 3–4 untouched |
...
2. **Untimed blocking calls on scheduler workers** (ADR-059 violations). **Two of the four
   are fixed 2026-09-01; two remain.**
   ...
   - ~~`%node-connect`'s unbounded DNS resolve on the worker~~ ✅ **FIXED** —
     ...
     traffic.
   - **Still open:** `proc-send`'s `write_all` under the per-child mutex (`subprocess.rs` —
     its comment cites `tcp-send`'s blocking contract, but `tcp-send` went async in
     ADR-143), and `read-line` holding the global stdin lock (`builtins/io.rs`). A handful
     of either still wedges the ~nproc-worker scheduler.
