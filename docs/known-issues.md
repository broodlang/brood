...
| KI-150 | **`base64` +6.6% at the 0.29.2 column refresh, ~5% of it the pre-run CHECK: `1a8759fb` removed `encoding`'s nine declared sigs and `brood file`'s pre-flight now infers them on every run** — bisected with fixed baselines and instruction counts (the row's wall is bimodal): `d464e9ed` 0.632G → `1a8759fb` 0.664G (+5%) → head 0.671G; with `BROOD_NO_CHECK=1` the wave's delta is +1%; A0/A1's own share is +1.6% (`a4b79219` 0.642G) | **OPEN 2026-09-16** — filed from the benchmark refresh, for the sig wave's owner. The KI-138/139 class: a checker cost is a runtime cost on this system because `brood file` checks before it runs, and the wave's premise ("the checker infers each verbatim without it") is true of the *verdict* and false of the *cost* — inference is work the declaration was saving, ~30M instructions on a program that touches `encoding`. Options, not decided here: keep sigs on the modules a short-lived program pays for (a per-module budget the `redundant-sigs` gate could measure), cache inferred signatures across runs the way the stdlib image caches bindings, or make the pre-flight skip inference for imaged std modules. Repro: `perf stat -e instructions:u brood bench/brood/base64.blsp` at the two commits, images live on both |
...
## KI-150 — `base64` +6.6%: the sig wave moved work from a declaration into every run's pre-flight check ⏳ OPEN 2026-09-16
...
**Not decided here** (the wave's owner's call): keep the sigs on modules a short-lived program
