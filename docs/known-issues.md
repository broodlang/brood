...
## KI-129 — a script outside `src/` loads a project module twice: rooted and bare ⚠️ OPEN
...
**Fix.** Not yet. The rule to install: a bare `(:use m)` evaluated in a project's directory
roots `m` when that project provides it, whether or not the file doing the using is the
project's — the same rooting the checker's require already applies, so the two agree.

**Guard (to write with the fix).** A `nest run` of a scratch file outside a fixture project's
`src/` that `(:use …)`s one of its modules, asserting `*features*` holds the rooted name only.
