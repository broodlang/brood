...
| KI-107 | **`tests/eval_server_test.blsp` — "`:all` traces, and cannot exceed the spy cap" fails with an EMPTY `:spy`**, about 1 run in 17 standalone | 🔍 **WATCH — two mechanisms found and fixed 2026-09-06; 6.0% → 0.67%, NOT closed.** The trace registry is shared globals, so two users of one traced name were invisible to each other: the second `trace-fn` is a documented no-op, and the first user's teardown then pulled the wrapper out from under it. Now refcounted — `debug/trace-hold`/`trace-release`, with a fresh install counting as its own first holder so an interactive trace cannot be stolen either. The parent-side `untrace-all` was the same bug one level up and is live in this file (the `answer` tests time out on purpose): holds are recorded per evaluator pid, and the parent releases exactly that child's. Measured against a same-N baseline: **9/150 → 3/450**, χ² = 16.0, p ≈ 0.00006. Three guards, each sabotage-verified to fail on EVERY run. What the numbers do not say: refcount vs the earlier scoped teardown is p ≈ 0.11, not established — the refcount ships because it closes a mechanism the scoped version provably leaves open, shown by a guard rather than by the rate. Residual 0.67% (3/450) with no mechanism currently named |
...
## KI-107 — `eval_server_test`'s `:all` cases are coupled through shared trace state 🔍 WATCH — two mechanisms found and fixed 2026-09-06, rate 6.0% → 0.67%, NOT closed
