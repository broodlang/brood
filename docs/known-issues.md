...
| KI-137 | **a strict verdict can depend on which OTHER modules are loaded when the file is checked, so the strict gate over std/ flickers with check order** — `math/clamp` (declared `-> number`, body `(math/max lo (math/min hi x))`) is clean in a fresh process and warns "body yields ordered" once `datetime` is loaded: `math/max` had no declaration, so its return was the kernel `%max` contract (number or comparable record), and the record half is inhabited only after a module registers comparable records. `nest check` checks files in parallel over one heap, so whether `datetime` is loaded when `math.blsp` is checked is a race: red twice on 2026-09-13 (a merged tree, a clean origin build), green in seven runs after, red again from the pre-push hook | ⏳ **WATCH 2026-09-13** — the symptom is fixed at the source: `math/max` and `math/min` declare `(& ?A -> ?A)` (the result IS an argument), so `clamp` types as `number` whatever is loaded. Open: the class — a file's verdict should not depend on the heap's load order. Either the gate loads the whole closure BEFORE checking any file (deterministic, and what a live image would see), or a kernel contract like `%max`'s carries the type variable itself. Repro: `(%check-strict! true) (reflect/check-file "std/math.blsp")` before and after `(require-one 'datetime)` at `820472eb`. Also fixed at the mechanism (881552a6): a module's own QUALIFIED definition of a name a by-name rule describes (`math/max` in `math.blsp`) reads that rule — the extremum result is its operands' union — ahead of the sig inferred over unknown parameters, so the instance is clean with or without the declaration; the class (any inferred return that is a registry-derived cover) stays open |
...
## KI-137 — a strict verdict depends on which other modules are loaded ⏳ WATCH 2026-09-13
...
**Done.** `(sig max (& ?A -> ?A))` and `(sig min (& ?A -> ?A))`: the result is one of the
arguments, so `(math/max lo hi)` over numbers is a number whatever else is loaded, and
`(math/max 2 3)` is `2 | 3`. The variadic type-variable form was verified on a scratch
module: a `string/length` of it warns, an `int` use is clean.
