...
| KI-162 | **`nest check --fix-sigs` writes each `sig` ABOVE its `defn`** — the placement `sig_placement.rs` forbids tree-wide, because under `BROOD_CONTRACTS=1` a `sig` is a rebinding of a name that must already exist | 🔶 **OPEN 2026-09-17** — insert after the definition's extent (the CST has it) and pin "directly below"; a downstream project running `--fix-sigs` today gets files contracts mode cannot load |
...
## KI-162 — `nest check --fix-sigs` writes each `sig` ABOVE its `defn`, which `sig_placement.rs` forbids tree-wide 🔶 OPEN 2026-09-17
...
**Fix.** The writer should insert AFTER the definition's last line — the CST has the form's
extent (`parse-source`), so no textual guessing — and the project test should pin "directly
BELOW". Noticed while making the writer decline an indented (wrapper-nested) head (B8's
session); not changed there, since it is a placement policy, not an order question.
