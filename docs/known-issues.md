...
## KI-165 — no `deftype` alias is enforced at runtime: an alias type accepts any value under `sig!` 🔶 OPEN 2026-09-18
...
**Fix, and the two parts of it.** The registry is already there — `reflect/type-aliases`
enumerates every alias — but it stores the type as a **string** (`"(record :x int :y int)"`),
so either the form is kept beside it or `type-matches?` reads the string. Then a
self-referential alias needs a binder so the lookup terminates: exactly what `%rec-unroll`
does for `(rec X …)`, with the alias's own name as the bound variable (the checker side of
that is C14 / ADR-369).

**Why it is filed rather than fixed here.** It is an ADR-327-wide hole, not C14's, and the
`else true` default it falls through is deliberate for types the contract genuinely cannot
interpret. Deciding that an alias is no longer in that class is the design call this entry
asks for.
