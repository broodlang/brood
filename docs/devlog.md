...
- **2026-06-28** — Raw-byte crypto/encoding layer (store-driver findings 2/3/4/5/6, all MED, now fixed). The stdlib's hash/crypto/encoding were string/hex/UTF-8-oriented, so SCRAM-SHA-256 forced ~150 lines of pure-Brood reimplementation costing ~2s/connection. Added raw-byte primitives — `%sha256-raw`/`-sha1`/`-sha384`/`-sha512`/`-md5` (byte vec → digest byte vec), `%hmac-sha256-raw`/`-sha1`/`-sha512` (byte-vec key+msg → byte vec), and `%pbkdf2-sha256-bytes` (byte-vec password+salt, microseconds vs ~2s) — with Brood wrappers (`hash/sha256-raw`, `hash/hmac-sha256-raw`, …); `crypto/pbkdf2` now coerces a string-or-bytes password/salt so a binary (base64-decoded) salt round-trips faithfully. base64/hex got pure-Brood byte-vector variants (`*-encode-bytes`/`*-decode-bytes` + URL-safe), via an `:invalid` decode sentinel so empty-valid stays distinct from failure. New `tests/scram_bytes_test.blsp` builds the whole SCRAM client-key chain over the new layer and checks it against RFC 7677 §3 (and across processes); known-answer vectors (FIPS/RFC 4231) added to hash/crypto/encoding tests. Also fixed a stale `chaos_test` case that still encoded the pre-`1a63eb7` spawn no-op (a crashing `(spawn (fn () …))` now propagates `[:error …]`, not `:normal`). **Follow-up audit** (three parallel sweeps for siblings of the fixed bug classes) turned up one: `remote-spawn`/`remote-spawn-sync` (std/prelude.blsp) wrapped their body in `(fn () …)` *unconditionally*, missing the `spawn--thunk-form?` guard `spawn`/`spawn-link` got in `1a63eb7` — so `(remote-spawn node (fn () body))` double-wrapped to `(fn () (fn () body))` and the receiver ran nothing (the same silent no-op, two layers deep). Fixed with the same guard; new `tests/remote_spawn_test.blsp` proves both the bare-expr and literal-lambda forms run against the local node. The stale-test sweep came back clean (the chaos_test fix was the only one; spawn change fully contained — only 5 literal-lambda spawns in the suite, all accounted for). The byte/UTF-8 sweep found no *new* bug class — its hits (`std/net/http.blsp` never sets binary mode so a binary HTTP body corrupts; subprocess `proc` stdio has no `proc-set-binary` toggle unlike sockets; `slurp`+package hashing can't read a binary asset) are all confirmed instances of the **known** Latin-1-carrier limitation (findings doc #7). **Then fixed all three** (keeping the Latin-1 byte-string carrier rather than waiting on a bytevector value kind): (1) `proc-set-binary` — mirrors `tcp-set-binary` for subprocess stdio (per-child flag, byte-faithful both directions), closing the socket/subprocess asymmetry; (2) `slurp-bytes` — byte-faithful file read → byte vector, and `package--sha256-file` now hashes bytes (`%sha256-bytes ∘ slurp-bytes`), so a package tree with a binary asset hashes instead of throwing — *identical* hash for text files, so no lockfile churn; (3) `http-read-request` reads in binary mode (exact framing + correct byte-counted `Content-Length`) then restores text mode before the response path. New coverage in `proc_test`/`slurp_bytes_test`/`http_test` (raw bytes through `cat`, binary-file hashing vs `sha256sum`, a binary POST body round-tripping end-to-end). `%sha256`-of-string and `bytes->str`-throwing stay by-design (byte-faithful counterparts already exist). The remaining #7 root — no bytevector kind, binary on a string carrier — stays a deliberate language-surface deferral.
